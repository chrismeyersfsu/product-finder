...
def query_listings(
    ...
    hidden with hide_listing() are omitted unless include_hidden. Each
    row carries pinned_at (set by pin_listing())."""
...
def pin_listing(listing_id: int) -> dict:
    """Pin a listing to the top of query_listings/best_deals and the Deals
    page. unpin_listing() reverses it. A hidden listing stays hidden even
    while pinned."""
    ok = storage.set_listing_pinned(_connect(), listing_id, True)
    return {"id": listing_id, "pinned": True} if ok else {"error": f"no listing: {listing_id}"}


def unpin_listing(listing_id: int) -> dict:
    """Unpin a listing."""
    ok = storage.set_listing_pinned(_connect(), listing_id, False)
    return {"id": listing_id, "pinned": False} if ok else {"error": f"no listing: {listing_id}"}
...
def backfill_ebay_sold(product_slug: str, query: str | None = None) -> dict:
    """Check eBay sold/completed listings against a product's criteria
    (eBay exposes roughly the last 90 days). Scores each match but does
    not persist any of them — there is no historical price store to put
    them in."""
    conn = _connect()
    product = storage.get_product(conn, product_slug)
    if not product:
    ...
    for q in queries:
        ...
        for li in result["listings"]:
            ...
            if scoring.rejected(
                ...
                skipped += 1
                continue
            _score_for(product, li["title"])
            matched += 1
    return {"matched": matched, "skipped_no_price_or_date": skipped, "errors": errors}
